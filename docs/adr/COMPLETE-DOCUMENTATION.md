...
> "I designed this as a reusable platform foundation — not a one-off demo.
> The namespace structure, Helm values in Git, and ArgoCD Application manifests
> are all designed so future projects plug in without touching core
> infrastructure. That's platform thinking: build once, enable many teams."

### On GitOps
> "I never ran kubectl apply manually to deploy the app. I pushed a YAML file
> to GitHub, ArgoCD detected the drift between desired state in Git and actual
> state in the cluster, and reconciled it automatically. selfHeal: true means
> if someone manually changes something in the cluster, ArgoCD reverts it.
> Git is the single source of truth."

### On the CI/CD pipeline
> "Every PR triggers a terraform plan that posts the exact infrastructure diff
> as a PR comment — reviewers see what changes before approving. Apply only
> runs on merge to main, with a manual approval gate via GitHub Environments.
> This is how you safely manage infrastructure changes at scale."

### On cost awareness
> "The cluster costs roughly $0.10/hour on Standard_B2s. I destroy it with
> terraform destroy when not working and recreate in 8 minutes. I treated
> free tier credits like a production budget — this mirrors how I approached
> cost optimisation at Capgemini."

### On the ADRs
> "Every architectural decision has a written ADR explaining the trade-off —
> not just what I chose but why, and what I gave up. ArgoCD over Flux,
> Prometheus over Azure Monitor, Helm over raw manifests. This is how
> platform teams document decisions so new engineers understand the context,
...
✅ Foundation complete
   ...
   · ArgoCD GitOps loop working
   · Prometheus + Grafana + Loki running
   · Sample app deployed via GitOps
   · GitHub Actions CI/CD pipeline live
   · Documentation + ADRs written
   · Screenshots taken

NEXT PHASE OPTIONS (pick based on energy + time)
────────────────────────────────────────────────

OPTION A — Project 2: FinOps Intelligence Engine
  Effort:    Medium (2–3 sessions)
  Stack:     Python · Azure Cost API · Streamlit · LLM API
  Builds on: This cluster (deploys as a pod in finops-engine namespace)
  AI angle:  LLM generates plain-English cost recommendations
  Interview: "I built a tool that catches cost anomalies before month-end
              and explains them in plain English using AI — not just alerts."

  Steps:
  1. Python script: pull Azure Cost Management API data
  2. Pandas: detect anomalies (rolling average + threshold)
  3. LLM call: "Here are cost anomalies this week — suggest specific fixes"
  4. Streamlit dashboard: live cost view + AI recommendations panel
  5. Dockerise + deploy to AKS via ArgoCD
  6. Connect to Grafana: cost metrics alongside cluster metrics

OPTION B — Project 3: Agentic AIOps
  Effort:    Medium-High (3–4 sessions)
  Stack:     Python · Prometheus API · Loki API · LLM API · GitHub API
  Builds on: Prometheus + Loki already running in this cluster
  AI angle:  Agent reads alert + logs → LLM reasons root cause →
             opens GitHub PR with draft runbook automatically
  Interview: "When an alert fires, my agent correlates metrics and logs,
              determines root cause using an LLM, and drafts a runbook
              as a GitHub PR — no human needed for the first response."

  Steps:
  1. Alertmanager webhook → Python FastAPI endpoint
  2. On alert: query Prometheus for related metrics (5-min window)
  3. On alert: query Loki for related logs (error lines, stack traces)
  4. Build LLM prompt: alert + metrics + logs → ask for root cause
  5. Parse LLM response → format as runbook markdown
  6. GitHub API: open PR to runbooks/ folder with the draft
  7. Deploy as a pod in AKS, triggered by real Alertmanager

OPTION C — Project 4: AI-Native IDP (biggest impact, most complex)
  Effort:    High (6–8 sessions)
  Stack:     Backstage · Terraform · GitHub Actions · Python · LLM API · OPA
  Builds on: Everything in this foundation
  AI angle:  After provisioning, LLM generates plain-English summary of
             what was created + flags any policy violations + suggests
             cost optimisations
  Interview: "Developers fill a form in Backstage, Terraform provisions
              a full AKS namespace with RBAC and resource quotas, OPA
              enforces cost and security policies, and an AI explains
              exactly what was created in plain English. Zero infra tickets."

  Steps:
  Phase 4a — Backstage portal (2 sessions)
  1. Install Backstage locally (npx @backstage/create-app)
  2. Create a Software Template: "Request a new environment"
     - Form fields: team name, environment type, resource size
  3. Template action: trigger GitHub Actions workflow with inputs
  4. GitHub Actions: run terraform with those inputs as variables

  Phase 4b — Policy enforcement (1 session)
  5. OPA/Conftest: write policies (max CPU limit, required labels,
     no privileged containers)
  6. Add OPA check as a pipeline gate — blocks if policy fails
  7. Write ADR-004: why OPA over Azure Policy

  Phase 4c — AI summary layer (1 session)
  8. After terraform apply: collect outputs (what was created)
  9. Call LLM: "Summarise what was provisioned, flag anything unusual,
     suggest cost optimisations"
  10. Post summary as GitHub PR comment + Slack notification

  Phase 4d — Backstage catalog (1 session)
  11. Register provisioned environment in Backstage software catalog
  12. Show ownership, tech stack, health status in Backstage UI

RECOMMENDED ORDER
─────────────────
Session next:  Project 2 (FinOps) — fastest visible output, AI angle clear
After that:    Project 3 (AIOps) — builds on live Prometheus/Loki data
Final:         Project 4 (IDP) — most impressive, needs Projects 2+3 done

WHY THIS ORDER
──────────────
· Each project adds AI intelligence to the existing stack
· Recruiters see a platform that grows — not isolated demos
· By Project 4, you can say: "The IDP provisions environments that are
  immediately monitored by the observability stack and tracked by the
  FinOps engine — it's a complete platform, not just tools."
```
